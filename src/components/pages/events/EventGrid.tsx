...
const EventGrid = ({ events, onEventView }: { events: IEvent[], onEventView: (event: IEvent) => void }) => {
    ...
    return (
        <div className="w-full max-w-screen-xl mx-auto pt-12 flex flex-col">
            <p className="mt-4 mb-6 w-full text-lg text-[#ffc300] font-semibold">All Events</p>
            <div className="grid grid-cols- sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 ">

                {events?.map((event) => (
                    <div
                        key={event?._id}
                        className="bg-white dark:bg-black border dark:border-gray-700  transition-shadow duration-300 cursor-pointer overflow-hidden"
